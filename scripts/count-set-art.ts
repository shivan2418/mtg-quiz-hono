...
const results = [...sets.entries()]
  .sort((a, b) => a[1].year.localeCompare(b[1].year))
  .map(([code, s]) => ({
    code,
    name: s.setName,
    year: s.year,
...
const output = {
  summary: {
    ...
    totalCards,
    totalUniqueArtwork: totalUnique,
  },
  sets: results,
};
...
for (const r of results) {
  console.log(`  ${r.code.padEnd(5)} ${r.name.padEnd(25)} ${String(r.totalCards).padStart(4)} cards  ${String(r.uniqueArtwork).padStart(4)} unique`);
}
console.log(`\nOutput: ${outPath}`);
